...
//テッセレーション使用しないバージョン、頂点を増やしたメッシュを用意してもらう必要がある
Shader "Custom/GrassLightVer"
{
    Properties
    {
		[Header(Shading)]
	...
	void geo(triangle vertexOutput IN[3] : SV_POSITION, inout TriangleStream<geometryOutput> triStream)
	{
		...
		for(int i = 0; i < BLADE_SEGMENTS; i++) 
		{
			...
			//ループすればするほど幅が小さくなる
			float segmentsWidth = width * (1 - t);

			//ループすればするほど高さが増える
			float segmentsHeight = height * t;
		...
		//最後に頭のほうに頂点一つ増やす、草のとがっているところのため
		triStream.Append(GenerateGrassVertex(pos, 0, forward, height, float2(0.5, 1), transformationMatrix));
	}
